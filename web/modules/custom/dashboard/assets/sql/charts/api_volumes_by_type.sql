SELECT
  d.date AS `Date`,
  CAST(YEAR(d.date) AS CHAR) AS `Year`,
  ...
  0 AS `Total_Volume`
FROM (
  SELECT DATE(CONCAT(:year, '-', LPAD(MONTH(STR_TO_DATE(:month, '%b')), 2, '0'), '-', LPAD(seq + 1, 2, '0'))) AS date
  FROM (
  ...
  WHERE seq < DAY(LAST_DAY(DATE(CONCAT(:year, '-', LPAD(MONTH(STR_TO_DATE(:month, '%b')), 2, '0'), '-01'))))
) d
CROSS JOIN (
...
SELECT
  ...
  CAST(YEAR(CAST(node__field_date.field_date_value AS DATETIME)) AS CHAR) AS `Year`,
  DATE_FORMAT(CAST(node__field_date.field_date_value AS DATETIME), '%b') AS `Month`,
  ...
  node__field_transaction_type.field_transaction_type_value AS `Transaction_Type`,
  SUM(node__field_api_volume_in_mil.field_api_volume_in_mil_value) AS `Total_Volume`
FROM node
...
LEFT JOIN node__field_transaction_type
  ON node__field_transaction_type.entity_id = node.nid
  AND node__field_transaction_type.revision_id = node.vid
...
WHERE node.type = 'analytics'
  [[AND taxonomy_term_field_data_0.name = :attribute]]
  ...
  [[AND DATE_FORMAT(CAST(node__field_date.field_date_value AS DATETIME), '%b') = :month]]
  [[AND CAST(YEAR(CAST(node__field_date.field_date_value AS DATETIME)) AS CHAR) = :year]]
GROUP BY
  `Date`,
  `Year`,
  `Month`,
