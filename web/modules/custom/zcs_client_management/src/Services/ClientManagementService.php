...
class ClientManagementService {
  ...
  public function createUpdateClientBillingProfile($client_name, $client_type, $client_status, $pricing_type) {
  ...
  public function createUpdateClientBilling($group) {
   ...
   public function createUpdateClientBillingV1($client_id, $client_name, $client_type,  $client_status,  $pricing_type) {
    $pricing_label = '';
    if ($pricing_type == 'international_pricing') {
      $pricing_label = 'international';
    }
  ...
  public function createUpdateClientBillingApproval($group, $data) {
    $client_id = $group->id();
    $client_name = $group->get('label')->value ?? '';
    $client_type = $group->get('field_partner_type')->value ?? '';
    $client_status = $group->get('field_partner_status')->value ?? '';
    ...
    $discount_attributes = [];
    if (!empty($data)) {
      foreach (Json::decode($data) as $key => $value) {
        $node = Node::load($key);
        $discount_attributes[] = [
          'attribute_name' => $node->get('title')->value,
          'client_name' => $client_name,
