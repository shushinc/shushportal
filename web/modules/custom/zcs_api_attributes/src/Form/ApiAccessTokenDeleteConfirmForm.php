...
/**
 * Confirmation form for deleting an API access token.
 */
class ApiAccessTokenDeleteConfirmForm extends ConfirmFormBase {
  ...
  /**
   * Constructs an ApiAccessTokenDeleteConfirmForm object.
   *
  ...
  public function getFormId() {
    return 'api_access_token_delete_confirm_form';
  }
  ...
  public function getQuestion() {
    return $this->t('Are you sure you want to delete the token "@name"?', [
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    if ($this->tokenService->deleteToken($this->tokenId)) {
      $this->messenger()->addStatus($this->t('Token "@name" has been deleted.', [
        '@name' => $this->token->name,
      ]));
    }
    else {
      $this->messenger()->addError($this->t('Failed to delete token.'));
    }
